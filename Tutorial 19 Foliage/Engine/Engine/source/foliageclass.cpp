...
bool FoliageClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* textureFilename, int fCount)
{
	bool result;
...
bool FoliageClass::Frame(XMFLOAT3 cameraPosition, ID3D11DeviceContext* deviceContext)
{
	...
	for(i=0; i<m_foliageCount; i++)
	{
		...
		angle = atan2(modelPosition.x - cameraPosition.x, modelPosition.z - cameraPosition.z) * (180.0 / 3.141592654f);
...
bool FoliageClass::LoadTexture(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* filename)
{
	bool result;
